...
export default function AllCredentials() {
  ...
  async function loadAllCredentials() {
    ...
    try {
      ...
      const allCredentials: CredentialInterface[] =
        await dataVault.getAllCredentialsOfUser();
      const extendedFiles: ExtendedCredentialInterface[] = allCredentials.map(
  ...
  async function DecryptCredentials(n: number) {
    try {
      const _decryptedMsg = await decryptMessage(
        allCredentials[n].password,
        web3ConnectionData.walletAddress
      );
      setAllCredentials(
        allCredentials.map((file: ExtendedCredentialInterface, i: number) => {
          if (i === n) {
  ...
  return (
    <div className="py-8">
      <Modal
        ...
        centered
      >
        <ShowCredentialsModel
          website={allCredentials[modelIndex]?.website || ""}
          usernameOrEmailOrPhone={
            allCredentials[modelIndex]?.usernameOrEmailOrPhone || ""
          }
          password={allCredentials[modelIndex]?.password || ""}
        />
      </Modal>

      <h2 className=" pl-8 font-poppins text-3xl text-hash-light underline">
        All Credentials of User
      </h2>

      <div className="itemContainer mt-8 flex gap-x-28">
        {isLoading ? (
          <Loader />
        ) : allCredentials.length > 0 ? (
          allCredentials.map((file, key) => (
            <OneCredentialItem
              key={key}
              index={key}
              openCredentialModel={openCredentialModel}
              website={file.website}
              usernameOrEmailOrPhone={file.usernameOrEmailOrPhone}
