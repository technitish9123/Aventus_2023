...
export default function AllFiles() {
  ...
  async function loadAllFiles() {
    ...
    try {
      ...
      const extendedFiles: ExtendedFileInterface[] = allFiles.map(
        (item: FileInterface) => {
    ...
    try {
      if (allFiles[n].advanceEncryptionStatus) {
        const _decryptedMsg = await decryptMessage(
          allFiles[n].decryptKey,
          web3ConnectionData.walletAddress
        );
        setAllFiles(
          allFiles.map((file: ExtendedFileInterface, i: number) => {
            if (i === n) {
              return {
                ...file,
                decryptKey: _decryptedMsg,
        ...
        setAllFiles(
          allFiles.map((file: ExtendedFileInterface, i: number) => {
            if (i === n) {
              return {
                ...file,
                fileHash: _decryptedMsg + "/" + file.fileName,
                decryptedStatus: true,
              };
            }
            return file;
  ...
  return (
    <div className="flex flex-col gap-y-16">
      ...
      <div className="itemContainer flex  justify-center gap-x-16 gap-y-8">
        {isLoading ? (
          <Loader />
        ) : allFiles.length > 0 ? (
          allFiles.map((file, key) => (
            <OneFileItem
              ...
              advanceEncryptionStatus={file.advanceEncryptionStatus}
